...
export default class RedeSocialController {
	constructor(redeSocial) {
		this.RedeSocial = redeSocial;
	}

	getAll() {
		return this.RedeSocial.findAll({})
			.then(result => defaultResponse(result))
			.catch(error => errorResponse(error.message));
	}

	getById(id) {
		return this.RedeSocial.findOne({ where: id })
			.then(result => defaultResponse(result))
			.catch(error => errorResponse(error.message));
	}

	create(data) {
		return this.RedeSocial.create(data)
	...
	update(data, id) {
		return this.RedeSocial.update(data, { where: id })
			.then(result => defaultResponse(result, httpsStatus.CREATED))
			.catch(error => errorResponse(error.message, httpsStatus.UNPROCESSABLE_ENTITY));
	}

	delete(params) {
		return this.RedeSocial.destroy({ where: params })
